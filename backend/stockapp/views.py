...
class GetData(APIView):
    ...
    def get(self, request, ticker):
        time.sleep(0.1)
        call = requests.get(
            "https://finnhub.io/api/v1/quote?symbol=%s&token=cb6avbiad3i70tu62u4g"
            % (ticker.upper())
        )
...
def historyHelper(user, ticker, count, buy):
    ...
    if user.order_history == None:
        ...
        user.save()
        print("t", user.order_history)
        print(user.order_history[now])
...
def buyShares(request, ticker, count):
    ...
    if cost <= user.balance:
        ...
        if existance:
            print("this stock exists already in profile")
